...
public class IssueUpdater {
   ...
   public IssueUpdater(Job<?, ?> project, JiraSite site, UpdaterSttings settings, PrintStream logger) {
      ...
      this.defaultService = createJiraRestService(site.userName, site.password);
   }
   ...
   private JiraRestServiceEx createJiraRestService(String userName, Secret password) {
      ...
      if ( StringUtils.isBlank(userName) || StringUtils.isBlank(password.getPlainText()) )
       ...
       int usedTimeout = site.timeout != null ? site.timeout : JiraSite.DEFAULT_TIMEOUT;
       return new JiraRestServiceEx(uri, jiraRestClient, userName, password.getPlainText(), usedTimeout);
   }
   ...
   public boolean updateIssue(IssueEx issue) {
      ...
      try {
         ...
         logger.println( String.format("Commenting failed on {0}. Carrying over to next build.", issue.id) );
   ...
   public void logWork(IssueEx issue) {
      ...
      if (service == null) {
         logger.println( String.format("Work log can not be added to %1$s because login from the commit author failed. " +
                                       "Issue will be commented.", issue.id) );
         comment(issue);
         return;
      }
